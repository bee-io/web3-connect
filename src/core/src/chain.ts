...
import { Chain, ProviderRpcErrorCode } from '../../common'
...
import { updateChain } from './store/actions'
...
async function setChain(options: {
  ...
  if (!chain) {
    throw new Error(
      `Chain with chainId: ${chainId} and chainNamespace: ${chainNamespace} has not been set and must be added when Web3Connect is initialized.`
    )
  }
