...
export const defaultNotifyEventStyles: Record<string, NotifyEventStyles> = {
  pending: {
    backgroundColor: 'var(--b-ee-primary-700, var(--primary-700))',
    borderColor: '#6370E5',
    eventIcon: hourglass
  },
  success: {
    backgroundColor: '#052E17',
    borderColor: 'var(--b-ee-success-300, var(--success-300))',
    eventIcon: checkmark
  },
  error: {
    backgroundColor: '#FDB1B11A',
    borderColor: 'var(--b-ee-danger-300, var(--danger-300))',
    eventIcon: errorIcon
  },
  hint: {
    backgroundColor: 'var(--b-ee-gray-500, var(--gray-500))',
    borderColor: 'var(--b-ee-gray-500, var(--gray-500))',
    iconColor: 'var(--b-ee-gray-100, var(--gray-100))',
    eventIcon: infoIcon
  }
}
...
const ETH_WALLET_REGEX = /(0x[a-f0-9]{40})/g;
const BTC_WALLET_REGEX = /^([13]{1}[a-km-zA-HJ-NP-Z1-9]{26,33}|bc1[a-z0-9]{39,59})$/;
const UNITS = {
  0: "wei",
  6: "mwei",
  9: "gwei",
...
export function isValidBTCAddress(wallet) {
  return BTC_WALLET_REGEX.test(wallet)
}
export function isValidAddress(wallet) {
  return Web3.utils.isAddress(wallet);
}
export function buildHash(...props) {
  return Web3.utils.keccak256(props.toString())
}
export function decimalsToUnit(decimals) {
  return UNITS[decimals] ? UNITS[decimals] : "ether";
}
export function formatWei(balance, decimals, fractionDigits) {
  let number;
  if(decimals === 8) {
    // number = Number((Web3.utils.fromWei(balance.toString(), 'gwei') * 10)).toFixed(fractionDigits);
  } else {
    number = Number(Web3.utils.fromWei(balance, decimalsToUnit(decimals))).toFixed(fractionDigits);
  }
  return numberToCurrency(number, fractionDigits);
}
export function getPriceByRate(balance, rate) {
  balance = balance ? Number(balance.toString().replace(/[^0-9.-]+/g,"")): 0;
  return numberToCurrency(balance / Number(rate));
}
export function numberToCurrency(number, fractionDigits= 2) {
  const formatNumber = new Intl.NumberFormat( 'en-US',{minimumFractionDigits:2, maximumFractionDigits: fractionDigits}).format(number);
  if (isNaN(parseFloat(formatNumber))) {
    return null;
  }
  return formatNumber === '0' ? '0.00' : formatNumber;
}
