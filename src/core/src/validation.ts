...
const chains = Joi.array()
  ...
  .error(e => {
    if (e[0].code === 'array.unique') {
      return new Error(
        `There is a duplicate Chain ID in your Web3Connect Chains array: ${e}`
      )
    }
    return new Error(`${e}`)
  })
...
const themeMap = Joi.object({
  '--be-e-background-color': Joi.string(),
  '--be-e-font-family': Joi.string(),
  '--be-e-foreground-color': Joi.string(),
  '--be-e-text-color': Joi.string(),
  '--be-e-border-color': Joi.string(),
  '--be-e-action-color': Joi.string(),
  '--be-e-border-radius': Joi.string()
})
